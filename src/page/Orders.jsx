...
import { getAllOrder, updateStatusOrder } from "../api/orderApi";
...
function Orders() {
  ...
  const handleUpdateStatus = async () => {
    ...
    try {
        ...
        if (response.code === 1000) {
             ...
             const updatedCurrentOrder = sorted.find(o => o.id === selectedOrder.id);
             if (updatedCurrentOrder) {
                 setSelectedOrder(updatedCurrentOrder); 
             }
        }
  ...
  const availableStatuses = selectedOrder ? getNextStatuses(selectedOrder.status) : [];
                                    ...
                                    {selectedOrder.status === "FAILED" && (
                                        <ListGroup.Item className="bg-danger bg-opacity-10">
                                            ...
                                            <p className="mb-0 mt-1 fst-italic">
                                                "{selectedOrder.errorNote || "Không có lý do cụ thể"}"
                                            </p>
                                        </ListGroup.Item>
                                    )}
                                </ListGroup>
                                ...
                                <Card.Body>
                                    <Form>
                                        <Form.Group className="mb-3">
                                            <Form.Label>Chuyển sang trạng thái:</Form.Label>
                                            <Form.Select 
                                                value={newStatus} 
                                            ...
                                            >
                                                ...
                                                {availableStatuses.map(st => (
                                                    <option key={st} value={st}>{statusMap[st]}</option>
                                                ))}
                                            </Form.Select>
                                            {availableStatuses.length === 0 && (
                                                <small className="text-muted d-block mt-2">Đơn hàng đã kết thúc.</small>
                                            )}
                                        </Form.Group>

                                        {newStatus === "FAILED" && (
                                            <Form.Group className="mb-3">
                                                <Form.Label className="text-danger fw-bold">Lý do hủy:</Form.Label>
                                                <Form.Control 
                                                    as="textarea" 
                                                    rows={3} 
                                                    value={note}
